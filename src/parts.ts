...
import {type WeeklyTime} from './types';
...
export const isValidWeeklyTimeParts = (wt: {day: number; hour: number; minute: number}): wt is WeeklyTimeParts => {
	if (Number.isNaN(wt.day) || Number.isNaN(wt.hour) || Number.isNaN(wt.minute)) {
		return false;
	}

	if (!(wt.day in dayNumberToCode)) {
		return false;
	}

	if (wt.hour < 0 || wt.hour > 23) {
		return false;
	}

	if (wt.minute < 0 || wt.minute > 59) {
		return false;
	}
